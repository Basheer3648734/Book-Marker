...
function Header() {
  ...
  const userProfileIcon = (
    <div className="ml-3 relative float-right z-10">
      <div>
      ...
      {showMenu ?
        <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5  dark:bg-gray-600 " >
          <button className="text-left w-full block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-500 dark:text-gray-100" onClick={goToAboutProfile}>Your Profile</button>

          <button className="text-left w-full block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-500 dark:text-gray-100" onClick={goToSetting}>Settings</button>
  ...
  return (
    <div className="container dark:bg-gray-800  w-11/12 lg:w-10/12 mx-auto flex justify-between mt-8 ">
      ...
      <button className="font-bold sm:text-xl md:text-2xl tracking-normal font-serif dark:text-gray-100 focus:outline-none" onClick={goToHome}>BookMarker</button>
      <div>
