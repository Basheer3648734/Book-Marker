...
function List({title,author,imageURL,lastRead,completedPercentage}) {
    ...
    return (
        <li onClick={e=>setShowOptions(prev=>!prev)} className="dark:text-gray-100 dark:bg-gray-700 mx-auto  mb-5 shadow-md text-gray-900 w-11/12 md:w-8/12 cursor-pointer truncate pr-1">
        <div className=" flex justify-even  items-center leading-3">
        <img src={imageURL.length>0?imageURL:defaultURL} className="mr-3 object-contain " style={{width:"125px", height:"125px"}}/>
        <div>
         ...
         {showOptions?(  <div  className="bg-gray-200 py-1.5 flex justify-end dark:bg-gray-500">
             <button  className="dark:text-red-800 dark:border-red-800 dark:hover:bg-red-300 border-2 border-red-600 text-red-600 rounded-md py-1 px-3 mr-2 font-medium text-sm hover:bg-red-100 hover:text-red-800 "><TrashBinIcon /> Delete</button>
       ...
       {showEdit?<EditForm id={title} hideForm={()=>setShowEdit(false)}/>:null}
        </li>
    )
}
